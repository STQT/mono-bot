...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Даты начала/конца интервала в таймзоне Asia/Tashkent
        tz = timezone.get_fixed_timezone(
            timezone.pytz.timezone(settings.TIME_ZONE).utcoffset(now).total_seconds() / 60
        ) if hasattr(timezone, "pytz") else timezone.get_current_timezone()

        start = datetime(year=int(year), month=int(month), day=1, tzinfo=tz)
        if month == 12:
            end = datetime(year=int(year) + 1, month=1, day=1, tzinfo=tz)
        else:
            end = datetime(year=int(year), month=int(month) + 1, day=1, tzinfo=tz)
        ...
        def export_for_type(code_type: str, filename: str):
            ...
            with open(path, "w", encoding="utf-8", newline="") as f:
                ...
                for qr in qs.iterator():
                    user = qr.scanned_by
                    scanned_at_utc = (
                        qr.scanned_at.astimezone(timezone.utc) if qr.scanned_at else None
                    )
                    scanned_at_local = (
                        qr.scanned_at.astimezone(tz) if qr.scanned_at else None
                    )
                    writer.writerow(
                        [
